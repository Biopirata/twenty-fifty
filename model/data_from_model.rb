...
class DataFromModel
  ...
  def calculate_pathway(code)
    ...
    { 
      ...
      'ghg_reduction_from_2000' => excel.output_ghg_percentage_reduction, # output.ghg.percentage.reduction in Excel
      'clean_generation_2050' => excel.output_nonfossil_percentage_generation, # output.nonfossil.percentage.generation in Excel
  ...
  def generate_example_pathways
    ...
    data = data.map do |pathway_data|
      {
        name: pathway_data[0],
        code: convert_float_to_letters(pathway_data[1..68]).join,
        description: wrap(pathway_data[69]),
        wiki: pathway_data[70],
        cost_comparator: (c = pathway_data[71]; c.is_a?(Numeric) ? c : nil )
      }
    end
  end
